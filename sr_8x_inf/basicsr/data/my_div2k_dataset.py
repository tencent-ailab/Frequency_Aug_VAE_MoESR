...
class SINGLEDIV2KDataset(data.Dataset):
    """FFHQ dataset.
    ...
    def __init__(self, opt):
        super(SINGLEDIV2KDataset, self).__init__()
        self.opt = opt
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            for idx, _ in enumerate(self.gt_folder):
                ...
                elif os.path.isfile(self.gt_folder[idx]): 
    ...
    def __getitem__(self, index):
        ...
        img_gt = augment(img_gt, hflip=self.opt['use_hflip'], rotation=False) # bgr
                
        # BGR to RGB, HWC to CHW, numpy to tensor
        if img_gt.shape[2] == 3:
            if img_gt.dtype == 'float64':
                img_gt = img_gt.astype('float32')
            img_gt = cv2.cvtColor(img_gt, cv2.COLOR_BGR2RGB)
        else:
            print("Color channel wrong!")
            NotImplementedError
...
@DATASET_REGISTRY.register()
class PAIREDDIV2KDataset(data.Dataset):
    """FFHQ dataset.
    ...
    def __init__(self, opt):
        super(PAIREDDIV2KDataset, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
    ...
    def __getitem__(self, index):
        ...
        img_lr = img2tensor(img_lr, bgr2rgb=True, float32=True) / 255.0
        ...
        return {'gt': img_gt, 'gt_path': gt_path, 'lr': img_lr_8x, 'lr_sr': img_lr_2x, 'lr_1x': img_lr}
